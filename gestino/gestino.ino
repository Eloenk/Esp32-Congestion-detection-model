/*
 * Smart Traffic Control System v3.0 - 4 Independent Lanes
 ...
 * - Web dashboard with real-time monitoring AND MANUAL CONTROL
...
#define CONGESTION_MODEL_QUADRATIC 1  // 🔴 CHANGE THIS to test different models
...
bool manualAllRed = false;
Lane manualActiveLane = ONE;
...
void setup() {
  ...
  logPrintln("\n╔════════════════════════════════════════════════════════╗");
  logPrintln("║  Smart Traffic Control System v3.0                    ║");
  ...
  // Start web server
  if (WiFi.status() == WL_CONNECTED) {
    setupWebServer();  // Added: This was missing
    httpServer.begin();
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (l.hasVehicles && l.waitStartTime > 0) {
      ...
      if (waitTime > MAX_WAIT_TIME) {
        ...
        logPrintln(getLaneName((Lane)i));
        return (Lane)i;
      }
    }
    
    // Skip empty lanes
    if (ENABLE_EMPTY_LANE_SKIP && !l.hasVehicles) continue;
...
void handleAllRedClearState() {
  ...
  if (elapsed >= ALL_RED_TIME) {
    ...
    logPrint(" | Duration: ");
    logPrintln(String(greenDuration / 1000.0) + "s");
  }
}
...
void handleManualLane() {
  if (httpServer.hasArg("id")) {
    int laneId = httpServer.arg("id").toInt();
    if (laneId >= 0 && laneId < 4 && lanes[laneId].enabled) {  // Improved: Check enabled
      manualMode = true;
      manualAllRed = false;
      manualActiveLane = (Lane)laneId;
      activeLane = manualActiveLane;  // Added: For UI consistency
      httpServer.send(200, "text/plain", "Manual Mode: Lane " + String(getLaneName(manualActiveLane)) + " Green");
      logPrintln("⚠️ MANUAL OVERRIDE: Lane " + String(getLaneName(manualActiveLane)) + " Forced Green");
      return;
    } else {
      httpServer.send(400, "text/plain", "Invalid or disabled lane ID");
      return;
    }
  }
  httpServer.send(400, "text/plain", "Missing or invalid lane ID");
}

void handleManualAllRed() {
  manualMode = true;
  manualAllRed = true;
  httpServer.send(200, "text/plain", "Manual Mode: ALL RED");
  logPrintln("⚠️ MANUAL OVERRIDE: ALL RED");
}

void handleResumeAuto() {
  manualMode = false;
  manualAllRed = false;
  currentState = DECISION;
  httpServer.send(200, "text/plain", "Resuming Auto Mode");
  logPrintln("✅ System Resumed Automatic Control");
}
...
void setupWebServer() {
  ...
  httpServer.on("/api/manual/lane", HTTP_POST, handleManualLane);
  httpServer.on("/api/manual/allred", HTTP_POST, handleManualAllRed);
  httpServer.on("/api/manual/auto", HTTP_POST, handleResumeAuto);
...
void handleAPIStatus() {
  ...
  json += "\"manualMode\":" + String(manualMode ? "true" : "false") + ","; // Added status
  if (manualMode && !manualAllRed) {  // Optional improvement: Add forcedLane
    json += "\"forcedLane\":\"" + String(getLaneName(manualActiveLane)) + "\",";
  }
...
String generateDashboardHTML() {
  // NOTE: Inserted MANUAL CONTROL section into your existing HTML structure
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .lane-card.active { border-left-color: #10b981; background: #ecfdf5; }
    .lane-name { font-size: 1.5em; font-weight: bold; margin-bottom: 15px; color: #333; }
    .lane-metric {
      display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid #eee;
    }
    .metric-value { font-weight: bold; color: #333; }
    .health-ok { color: #10b981; } .health-fault { color: #ef4444; }
    
    /* NEW: Manual Control Styles */
    .controls {
      background: white; padding: 20px; border-radius: 10px;
      box-shadow: 0 5px 15px rgba(0,0,0,0.1); margin-bottom: 20px; text-align: center;
    }
    .btn-group { margin: 15px 0; }
    button {
      background: #667eea; color: white; border: none; padding: 12px 30px;
      border-radius: 5px; font-size: 1em; cursor: pointer; margin: 5px;
      transition: all 0.3s;
    }
    button:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(102, 126, 234, 0.4); }
    .btn-red { background: #ef4444; } .btn-red:hover { background: #d32f2f; }
    .btn-green { background: #10b981; } .btn-green:hover { background: #059669; }
    
    .status-badge {
      display: inline-block; padding: 5px 15px; border-radius: 20px;
      font-size: 0.9em; font-weight: bold; margin-top: 10px;
    }
    ...
    .status-manual { background: #ef4444; color: white; border: 2px solid #fff; }
...
<body>
  <div class="container">
    ...
    <div class="controls">
      ...
      <div class="btn-group">
        <button onclick="setManualLane(0)">Force Lane ONE</button>
        <button onclick="setManualLane(1)">Force Lane TWO</button>
        <button onclick="setManualLane(2)">Force Lane THREE</button>
        <button onclick="setManualLane(3)">Force Lane FOUR</button>
      </div>
      <div class="btn-group">
        <button class="btn-red" onclick="setAllRed()">🛑 STOP ALL (RED)</button>
        <button class="btn-green" onclick="resumeAuto()">🤖 RESUME AUTO</button>
      </div>
    </div>
  ...
  <script>
    function updateDashboard() {
      fetch('/api/status')
        ...
        .then(data => {
          ...
          if (data.manualMode) {
             stateElement.textContent = "MANUAL OVERRIDE";
             stateElement.className = 'status-badge status-manual pulse';
          } else {
             stateElement.textContent = data.state;
             stateElement.className = 'status-badge';
             if (data.state === 'GREEN') stateElement.classList.add('status-green', 'pulse');
             else if (data.state === 'YELLOW') stateElement.classList.add('status-yellow');
             else stateElement.classList.add('status-red');
          }
          ...
          data.lanes.forEach(lane => {
            ...
            laneCard.innerHTML = `
              ...
              <div class="lane-metric"><span class="metric-label">Current Count:</span><span class="metric-value">${lane.count}</span></div>
              <div class="lane-metric"><span class="metric-label">Total Entered:</span><span class="metric-value">${lane.entered}</span></div>
              <div class="lane-metric"><span class="metric-label">Total Exited:</span><span class="metric-value">${lane.exited}</span></div>
              <div class="lane-metric"><span class="metric-label">Priority Score:</span><span class="metric-value">${lane.priority.toFixed(2)}</span></div>
              <div class="lane-metric"><span class="metric-label">Sensor Health:</span>
                <span class="metric-value ${lane.sensorHealth ? 'health-ok' : 'health-fault'}">${lane.sensorHealth ? '✓ OK' : '✗ FAULT'}</span>
              </div>
            `;
    ...
    // Manual API Calls
    function setManualLane(id) { fetch('/api/manual/lane?id=' + id, { method: 'POST' }); }
    function setAllRed() { fetch('/api/manual/allred', { method: 'POST' }); }
    function resumeAuto() { fetch('/api/manual/auto', { method: 'POST' }); }

    function downloadCSV() { window.location.href = '/api/csv'; }
    function clearCSV() { if(confirm('Clear CSV log?')) fetch('/api/clear_csv', { method: 'POST' }); }
...
void loop() {
  ...
  // === INSERTED: MANUAL OVERRIDE CHECK ===
  if (manualMode) {
    setAllLanesRed();
    if (manualAllRed) {
      logPrintln("⚠️ MANUAL OVERRIDE: All lanes RED");  // Added: Logging
    } else {
      setLaneColor(manualActiveLane, 'G');
      logPrintln("⚠️ MANUAL OVERRIDE: Lane " + String(getLaneName(manualActiveLane)) + " FORCED GREEN");  // Added: Logging
    }
    // Skip State Machine
    delay(50);
    return;
  ...
  switch (currentState) {
    case IDLE: currentState = DECISION; stateStartTime = millis(); break;
    case DECISION: handleDecisionState(); break;
    case TRANSITION_YELLOW: handleTransitionYellowState(); break;
    case ALL_RED_CLEAR: handleAllRedClearState(); break;
    case ACTIVE_GREEN: handleActiveGreenState(); break;
  }
